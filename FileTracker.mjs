...
class Filetracker {
    ...
    this.objectsPath = path.join(this.repoPath, 'objects'); // .FileTracker/objects
    this.headPath = path.join(this.repoPath, 'HEAD'); // .FileTracker/HEAD
    this.indexPath = path.join(this.repoPath, 'index'); // .FileTracker/index
        this.init();
    }
    ...
    async init() {
        ...
        try {
            ...
            console.log("Already initialised the .FileTracker folder");
        }
    }
    ...
    const newFileHashedObjectPath = path.join(this.objectsPath, fileHash); // .FileTracker/objects/abc123
...
program.command('add <file>').action(async (file) => {
    const filetracker = new Filetracker();
    await filetracker.add(file);
});

program.command('commit <message>').action(async (message) => {
    const filetracker = new Filetracker();
    await filetracker.commit(message);
});

program.command('log').action(async () => {
    const filetracker = new Filetracker();
    await filetracker.log();
});

program.command('show <commitHash>').action(async (commitHash) => {
    const filetracker = new Filetracker();
    await filetracker.showCommitDiff(commitHash);
});
